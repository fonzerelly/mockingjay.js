...
describe('server', () => {
    ...
    describe('db datas in different modes', () => {
        ...
        beforeEach(() => {
            db = {
                'http%3A%2F%2Fwww.already-stored-url.com%3Fparam%3D42': {
                    'foo': 'bar'
                }
            }
            instance = server.init(db, port)
        ...
        describe('READONLY', () => {
            ...
            it('should GET http://www.already-stored-url.com?param=42', (done) => {
                chai.request(instance)
                    .get('/http%3A%2F%2Fwww.already-stored-url.com%3Fparam%3D42')
                    .end((err, res) => {
                        expect(err).toBeNull()
                        res.should.have.status(200)
            ...
            it('should fail on GET http://www.not-yet-stored-url.com?param=42', (done) => {
                chai.request(instance)
                    .get('/http%3A%2F%2Fwww.not-yet-stored-url.com%3Fparam%3D42')
                    .end((req, res) => {
